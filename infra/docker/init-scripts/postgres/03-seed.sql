...
--
-- Copie de phase-0/database/07-seed-data.sql pour exécution automatique
-- par le container Docker au premier démarrage.
--
-- IMPORTANT : ce script ne s'exécute QUE en développement local Docker.
-- Ne JAMAIS exécuter en production (pollue le catalogue avec des données test).
--
-- Ce seed crée :
...
--   - 1 préférence membre
--   - 3 recettes d'exemple avec embeddings factices (vecteurs 0.001)
--   - 1 plan hebdomadaire de test avec 3 repas planifiés
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- EMBEDDINGS FACTICES (vecteurs 0.001 pour les tests)
-- Un vecteur tout à zéro provoquerait une division par zéro dans vector_cosine_ops.
-- 0.001 évite ce problème tout en restant "presque nul" (inutilisable pour la prod).
-- AVANT les tests de recommandation, régénérer avec le vrai modèle all-MiniLM-L6-v2.
-- -----------------------------------------------------------------------------
...
BEGIN
    ...
    RAISE NOTICE '03-seed.sql OK : % ingrédients, % recettes, % embeddings.',
